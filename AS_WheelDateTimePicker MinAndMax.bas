...
Sub Class_Globals
	...
	Private m_WeekDays As Boolean'ignore
...
Private Sub FillWheelPicker
	If m_PickerType = "TimePicker" Then
		...
		For i = 0 To IIf(m_AMPM = True,12,23) -1
		...
		xwp_Wheel.SelectRow(IIf(m_Date,1,0),m_Hour,False)
		xwp_Wheel.SelectRow(IIf(m_Date,2,0),m_Minute,False)
...
Private Sub AddMonths
	...
	'xwp_Wheel.RemoveColumn(0)
	xwp_Wheel.SetItems(0,lstMonths)
...
Private Sub AddDatePicker
	...
	Dim tmp_lst As List : tmp_lst.Initialize
	For i = 1 To 32 -1
		tmp_lst.Add(xwp_Wheel.CreateASWheelPicker_Item(NumberFormat(i,2,0),i))
	Next
	...
	xwp_Wheel.SelectRow2(0,DateTime.GetMonth(m_StartDate),True)
	xwp_Wheel.SelectRow2(1,DateTime.GetDayOfMonth(m_StartDate),True)
	xwp_Wheel.SelectRow2(2,DateTime.GetYear(m_StartDate),True)
	
	ItemChange(0,xwp_Wheel.GetIndex(0),False)
	ItemChange(1,xwp_Wheel.GetIndex(1),False)
	ItemChange(2,xwp_Wheel.GetIndex(2),False)
...
Private Sub AddDate2TimePicker As ResumableSub
	...
	Dim DayCount As Int = Max(1, DateUtils.PeriodBetweenInDays(StartDate,IIf(m_MaxDate > 0,m_MaxDate, DateUtils.SetDate(DateTime.GetYear(m_StartDate)+YearGap,12,31))).Days)
	Dim StartIndex As Int = 0
	...
	For i = 0 To DayCount -1
		...
		Dim CurrentDate As Long = StartDate + DateTime.TicksPerDay*(i)
		'Log(DateUtils.TicksToString(CurrentDate))
		Dim Text As String = ""
...
Public Sub setMinute(Minute As Int)
	...
	If m_PickerType = "TimePicker" Then
		...
		Sleep(0)
		#End If
		xwp_Wheel.SelectRow(IIf(m_Date,2,1),Minute,False)
...
Public Sub setDate(Date As Long)
	If m_PickerType = "TimePicker" And m_Date Then
		xwp_Wheel.SelectRow(0,Date,False)
	Else if m_PickerType = "DatePicker" Then

		Dim MinimumYear As Int = xwp_Wheel.GetItem(2,0).Value
		
		xwp_Wheel.SelectRow(0,DateTime.GetMonth(Date) -1,False)
		xwp_Wheel.SelectRow(1,DateTime.GetDayOfMonth(Date) -1,False)
		xwp_Wheel.SelectRow(2,DateTime.GetYear(Date) - MinimumYear,False)
...
Public Sub getDate As Long
	...
	Else if m_PickerType = "DatePicker" Then
		Date = DateUtils.SetDate(xwp_Wheel.GetSelectedItem(2).Value,xwp_Wheel.GetSelectedItem(0).Value,xwp_Wheel.GetSelectedItem(1).Value)
	End If
	Return Date
End Sub
...
Private Sub ItemChange(Column As Int,Row As Int,Event As Boolean)'ignore
	...
	If m_PickerType = "DatePicker" Then
		...
		If Column = 2 Or Column = 0 Then
			...
			If m_MaxDate > 0 And xwp_Wheel.GetSelectedItem(2).Value.As(Int) = DateTime.GetYear(m_MaxDate) And xwp_Wheel.GetSelectedItem(0).Value.As(Int) = DateTime.GetMonth(m_MaxDate) Then
				NumerOfDaysInMonth = DateTime.GetDayOfMonth(m_MaxDate)
			End If
			
			'Monats Wheel
			If m_MaxDate > 0 And xwp_Wheel.GetSelectedItem(2).Value.As(Int) = DateTime.GetYear(m_MaxDate) Then
				
				For i = (xwp_Wheel.GetListView(0).Size-1) -1 To (DateTime.GetMonth(m_MaxDate)+1) Step -1
					xwp_Wheel.RemoveItemAt(0,i)
				Next
			else If m_MinDate > 0 And xwp_Wheel.GetSelectedItem(2).Value.As(Int) = DateTime.GetYear(m_MinDate) Then
				'Log(DateTime.GetMonth(m_MinDate))
				If xwp_Wheel.GetItem(0,0).Value < DateTime.GetMonth(m_MinDate) Then
					Dim Month As Int = xwp_Wheel.GetSelectedItem(0).Value.As(Int)
					For i = DateTime.GetMonth(m_MinDate) -1 To 1 Step -1
						xwp_Wheel.RemoveItemAt(0,i -1)
					Next
					xwp_Wheel.SelectRow2(0,Month,False)
				End If
			Else if xwp_Wheel.Size(0) < 12 Then
				Dim Month As Int = xwp_Wheel.GetSelectedItem(0).Value.As(Int)
				AddMonths
				xwp_Wheel.SelectRow2(0,Month,False)
			End If
			...
			Else
				...
				'Log(xwp_Wheel.GetItem(1,xwp_Wheel.Size(1) -1).Value)
				'Log(NumerOfDaysInMonth)

				'Log(xwp_Wheel.Size(1) -1)

				If xwp_Wheel.GetItem(1,xwp_Wheel.Size(1) -1).IsInitialized And xwp_Wheel.GetItem(1,xwp_Wheel.Size(1) -1).Value > NumerOfDaysInMonth Then
					
					For i = (xwp_Wheel.GetListView(1).Size-1) -1 To (NumerOfDaysInMonth) Step -1
						xwp_Wheel.RemoveItemAt(1,i)
					Next
					
				End If
				
				'Log(DateUtils.TicksToString(m_MaxDate))
		
				
			End If
			
			If m_MinDate > 0 And xwp_Wheel.GetSelectedItem(2).Value.As(Int) = DateTime.GetYear(m_MinDate) And xwp_Wheel.GetSelectedItem(0).Value.As(Int) = DateTime.GetMonth(m_MinDate) And xwp_Wheel.GetItem(1,0).Value < DateTime.GetDayOfMonth(m_MinDate) Then
					
				'Log(xwp_Wheel.GetItem(1,0).Value)
				'Log(DateTime.GetDayOfMonth(m_MinDate))
				
				Dim isLower As Boolean = IIf(xwp_Wheel.GetItem(1,xwp_Wheel.GetIndex(1)).Value < DateTime.GetDayOfMonth(m_MinDate),True,False)
				Dim DayOfMonth As Int = xwp_Wheel.GetItem(1,xwp_Wheel.GetIndex(1)).Value
				
				For i = DateTime.GetDayOfMonth(m_MinDate) -2 To 0 Step -1
					xwp_Wheel.RemoveItemAt(1,i)
				Next
					
				If isLower Then
					xwp_Wheel.SelectRow(1,0,False)
				Else
					xwp_Wheel.SelectRow2(1,DayOfMonth,False)
				End If
					
			Else If m_MinDate > 0 And xwp_Wheel.GetItem(1,0).Value > 1 Then
				Dim TargetValue As Int = xwp_Wheel.GetItem(1,0).Value
				Dim DayOfMonth As Int = xwp_Wheel.GetItem(1,xwp_Wheel.GetIndex(1)).Value
...
Private Sub SelectedTimeChanged
	If xui.SubExists(mCallBack, mEventName & "_SelectedTimeChanged", 2) And m_PickerType = "TimePicker" Then
		CallSub3(mCallBack, mEventName & "_SelectedTimeChanged",xwp_Wheel.GetSelectedItem(IIf(m_Date,1,0)).Value,xwp_Wheel.GetSelectedItem(IIf(m_Date,2,1)).Value)
	End If
End Sub
